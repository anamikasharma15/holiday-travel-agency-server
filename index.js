...
async function run (){
    try{
        ...
        const database = client.db('holiday-travel-agency');
      ...
      app.get('/singleServices/:id', async(req, res) => {
        ...
        const singleServices= await blogsCollection.findOne(query);
      ...
      app.delete('/deleteProduct/:id', async(req,res) => {
        const id = req.params.id;
        const query = {_id: ObjectID(id)}
        const deleteProduct = await blogsCollection.deleteOne(query);
...
app.get('/', (req, res) => {
  res.send('Hello HOILDAY travel agency!')
})
